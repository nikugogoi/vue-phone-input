  ...
  <div class="intl-phone-input allow-dropdown" v-on:mouseleave="hideList">
    <div class="flag-container" v-on:click.stop="showList">
      ...
      <ul class="country-list" v-if="isVisiblePanel">
        <li v-for="item in itemsData.frontItems" v-bind:class="`country${item.code === phoneCode ? ' highlight' : ''}`" v-on:click.stop="setCode(item.code)">
          <div class="flag-box">
        ...
        <li v-for="item in itemsData.otherItems" v-bind:class="`country${item.code === phoneCode ? ' highlight' : ''}`" v-on:click.stop="setCode(item.code)">
          <div class="flag-box">
            <div v-bind:class="`iti-flag ${item.code}`"></div>
          </div>
          <span class="country-name">{{ item.name }}</span>
    ...
    <input type="text" class="form-control" v-bind:name="name" autocomplete="off" v-on:input="handleChangePhoneNumber" v-bind:value="phoneNumber" :placeholder="currentPhoneData.phoneFormat" />
  </div>
</template>
...
<script>
  ...
  let counter = 0;
  const defaultName = () => {
    const returnName = `tel-input-${counter}`;
    counter += 1;
    return returnName;
  };
